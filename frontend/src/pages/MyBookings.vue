<template>
  <div class="container">
    ...
    <div v-else class="grid gap-4">
      <div v-for="booking in bookings" :key="booking.id" class="card flex flex-col md:flex-row md:items-center justify-between border-yellow-200">
        <div>
          <h2 class="text-xl font-bold text-gray-900 mb-1">{{ booking.movie.title }}</h2>
          <div class="flex flex-wrap items-center text-sm text-gray-600 gap-y-1 gap-x-4">
            <span class="flex items-center whitespace-nowrap">🕒 {{ booking.movie.show_time ? new Date(booking.movie.show_time).toLocaleString() : 'TBD' }}</span>
            <span class="flex items-center whitespace-nowrap">🪑 {{ booking.seats_booked }} Seats Booked</span>
