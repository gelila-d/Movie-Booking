  ...
  <nav class="bg-white border-b border-yellow-200 px-6 py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
    ...
    <div class="flex items-center space-x-4">
      <template v-if="user">
        ...
        <button @click="logout" class="text-sm text-yellow-800 bg-yellow-200 hover:bg-yellow-300 px-4 py-2 rounded-lg transition-colors border border-yellow-300 font-bold">
          Logout
        </button>
      </template>
      <template v-else>
        <router-link to="/login" class="text-sm text-gray-600 hover:text-yellow-600 transition-colors font-medium">Login</router-link>
        <router-link to="/register" class="text-sm text-white bg-yellow-500 hover:bg-yellow-600 px-4 py-2 rounded-lg transition-colors font-bold shadow-sm">
          Register
        </router-link>
      </template>
    </div>
...
<style scoped>
/* Simplified style block to avoid build issues */
</style>